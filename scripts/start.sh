...
# Two canonical canvases, with recoverable backups outside ComfyUI's list.
...
"${PYTHON_BIN}" /opt/runpod-wan-animate/scripts/install_workflows.py \
  --source "${WORKFLOW_SRC}" --destination "${WORKFLOW_DST}" \
  --backup "${WORKSPACE_DIR}/workflow-backups" \
