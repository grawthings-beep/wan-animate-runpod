...
既定の`loop-quality`は18 assets、約43.08 GBです。4本をサイズ順に並列取得します。
...
ダウンロード前に、選択profileの未取得容量と12 GBの作業・出力余白を確認します。`loop-quality`では約55.08 GB以上の空きが必要です。テンプレートはVolume Disk 100 GBにしてください。容量不足なら1 byteも取得する前に停止し、`Disk quota exceeded`を防ぎます。
...
| `loop-quality` | 18 | 43.08 GB | Smooth v6の音なしシームレスループ＋NSFW-22＋SmoothXXXAnimation＋Anime Cumshot Aesthetics＋iroiroLoRA High 5本 |
| `t2v-quality` | 7 | 40.68 GB | Smooth v6のT2V |
| `full` | 45 | 147.59 GB | 両ワークフローの全asset |

`loop-quality`が既定です。First-to-Last Frameの実行経路だけを残し、LightX2V rank128、NSFW-22 High/Low、SmoothXXXAnimation High/Low、Anime Cumshot Aesthetics High/Low、iroiroLoRA High 5本を取得します。別ブランチのLoRA、未接続GGUF、MMAudioはダウンロードもworkflow表示も行いません。`lightning-longvideo`を明示した場合だけNative Enhanced Lightning用assetを取得します。
...
- `cheek_bulge_fellatio_high`: High側のみ、triggerは`cheek bulge, fellatio`。1.0で既定OFF
- `glans_licking_high`: High側のみ、triggerは`glans licking`。1.0で既定OFF
- `head_back_high`: High側のみ、triggerは`head back`。1.0で既定OFF
- `paizuri_unaligned_breasts_high`: High側のみ、triggerは`paizuri, unaligned breasts`。1.0で既定OFF
- `washizukami_high`: High側のみ、作者記載のtriggerは`grabbin own/another's breast, deep skin`。1.0で既定OFF

指定どおりLightX2V High 3.0 / Low 1.5、NSFW-22 High 2.75 / Low 1.65、SmoothXXXAnimation High 1.5 / Low 1.0を同時にONにし、既定解像度を528×704に固定しています。Anime Cumshot AestheticsはSmoothMixとの組み合わせをユーザーが明示的に試す場合だけHigh/LowをONにしてください。iroiroLoRA 5本はHigh側だけに追加しており、必要な効果を1本ずつONにするのが基本です。private Hugging Face backupの6本には`HF_TOKEN`が必要ですが、公開iroiroLoRA 5本の取得には不要です。
