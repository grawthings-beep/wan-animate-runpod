...
これはJupyter、SSH、FileBrowser、Managerと、ループに不要なMMAudio、GGUF、
長尺動画系custom nodeを除いた5.523 GiBのimageです。AIO / Native Lightning用の
別Templateだけ、従来の`sha-<40文字のcommit SHA>`（全入りimage）を使います。
...
MODEL PROFILE: loop-quality (26 assets)
DISK PREFLIGHT: path=/workspace/comfyui free=... download_remaining=46.13 GB headroom=12.00 GB
TRANSFER ENGINE: 4 files in parallel
...
[check_env] profile=loop-quality assets=26 required_missing=0 optional_missing=0
```
...
ループ用は`ghcr.io/grawthings-beep/wan-animate-runpod:minimal-sha-<40文字のcommit SHA>`、
...
新規Podは約46.13 GBです。途中でPodを停止しても、同じローカルVolume DiskならHF cacheまたは`.part`から再開します。PodをTerminateした場合は削除され、次の新規Podで再取得します。`required_missing=0`になった後、ConnectからHTTP Service Port 8188を開きます。
