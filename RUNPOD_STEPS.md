...
MODEL PROFILE: loop-quality (18 assets)
DISK PREFLIGHT: path=/workspace/comfyui free=... download_remaining=43.08 GB headroom=12.00 GB
TRANSFER ENGINE: 4 files in parallel
...
[check_env] profile=loop-quality assets=18 required_missing=0 optional_missing=0
```
...
新規Podは約43.08 GBです。途中でPodを停止しても、同じローカルVolume DiskならHF cacheまたは`.part`から再開します。PodをTerminateした場合は削除され、次の新規Podで再取得します。`required_missing=0`になった後、ConnectからHTTP Service Port 8188を開きます。
